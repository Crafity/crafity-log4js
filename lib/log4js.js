...
function configureAppenders(appenderList, keepAppenders) {
	if (!keepAppenders) { clearAppenders(); }
	if (appenderList) {
		appenderList.forEach(function (appenderConfig) {
			var appender = loadAppender(appenderConfig.type, appenderConfig.console);
			appender.name = appenderConfig.name || appender.name;
			appenderConfig.makers = appenderMakers;
			appender = appenderMakers[appenderConfig.type](appenderConfig);
			if (appender) {
				appender.categoryName = appenderConfig.category;
...
function configureOnceOff(config, keep) {
	if (config) {
		try {
			configureAppenders(config.appenders, keep);
...
function configure(configurationFileOrObject, options) {
	...
	if (config === undefined || config === null || typeof(config) === 'string') {
		...
		configureOnceOff(loadConfigurationFile(config), options.keep);
	} else {
		options = options || {};
		if (options.reloadSecs) {
			getLogger('log4js').warn('Ignoring configuration reload parameter for "object" configuration.');
		}
		configureOnceOff(config, options.keep);
	}
}
...
function loadAppender(appender, captureConsole) {
	if (appender === "console" || captureConsole) {
		replaceConsole();
	}
...
module.exports = {
	...
	configure: configure,
	configureAppenders: configureAppenders,
